...
7. **环境阻塞（影响本轮最后一步的复跑）**：从 17:24 起云库连接数被**外部客户端**打满
   ...
   **因此**：
   ...
   - ❌ **未能复跑的**：第 2 轮 `verify.ps1` **跑完之后**再跑一次 `sql/09` 与一次库侧原始计数
     ...
     （命令见 ⑤ 附），我不把它写成「已验证」。
   - **后台重试已跑满并失败**：`.tmp-b31/retry-final-check.ps1`（每 20 秒一次、共 **40 次**，
     17:39:14 → 17:52:47）**全部**报 `too many clients already`，最后一行 `FINAL-CHECK-FAILED`。
     即云库从 17:24 起**持续打满至少 28 分钟未恢复**，不是瞬时抖动。
     该脚本保留在 `.tmp-b31/`，库恢复后可直接复跑（它会自动写
     `.tmp-b31/final-db-counts.txt` 与 `.tmp-b31/sql09-final.txt`）。
