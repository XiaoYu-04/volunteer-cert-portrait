# -*- coding: utf-8 -*-
"""
把 docs/论文/ 下的 Markdown 合并导出为 论文初稿.docx。

合稿顺序：摘要 → 绪论 → 01 需求分析 → 02 概要设计 → 03 详细设计 → 04 系统实现 → 05 系统测试
→ 结论与展望 → 参考文献 → 致谢 → 附录（章节号由 Word 排版时统一处理，本脚本不编号）。
文件缺失时跳过并打印 [skip]，所以新增章节未写完时也能先导出已有部分。
...
CHAPTERS = [
    # 合稿顺序（2026-09-25 补：新增的 6 块不参与 01~05 的编号，见 docs/论文/README.md 的「合稿顺序」）
    "00-摘要与关键词.md",
    "绪论.md",
    "01-需求分析.md",
    "02-概要设计.md",
