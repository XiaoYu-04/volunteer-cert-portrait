...
| 后端接口 | **59 条路径 / 72 个「方法 + 路径」** | `docs/测试报告.md:16` |
| 前端页面 | **29 个** | `docs/前端进展与待办.md:34` |
| 前端接口调用 | **70 个** | `docs/测试报告.md:16` |
| 前端手写组件 | **21 个** | `docs/前端进展与待办.md:68-76` |
| ECharts 图表 | **13 个** | 同上 `:78-84` |
| mock 接口 | **60+ 个** | 同上 `:91` |
| 后端单元测试 | **0 个**（9 个模块 `src/test` 全为空目录） | `docs/测试报告.md:204` |
...
**接口**：**18 个**，覆盖 `/activities`、`/categories`、`/signups`、`/attendance`
（同上 `:239`）。
...
| 手动重算 | `POST /api/v1/portraits/recompute`，**只写真正变化的行** | `docs/测试报告.md:87-88` |
...
| `POST /portraits/recompute` | `scanned=1503, updated=3` | `docs/测试报告.md:87` |
...
**`sql/07` 的末尾自检输出（2026-09-25 实跑，逐字）**（`docs/测试报告.md:294`）：
...
出处：`docs/测试报告.md:15`、`:283-291`、`docs/待办清单.md:847-849`、
`docs/前端进展与待办.md:137`。

> ⚠️ **答辩材料里的数字必须标明是哪一套**（`docs/待办清单.md:849`）。
> 实测现存 `dist/` 就是 **mock 口径**（`dist/assets/` 里有 `mock-DAfFHmNx.js`），
> 直接拿它截图会**串口径**（同上 `:850-851`）。

### 4.7.4 演示数据的还原与踩坑

**端到端测试会写入真实数据**，测试后用 `cleanup.sql` + `fix7.sql` 完全还原并复核
（`docs/测试报告.md:272-281`）：
...
**还原过程中发现的机制问题**（`docs/测试报告.md:297-299`）：
...
出处：`docs/测试报告.md:11`、`:17`、`:26-28`、`:112`、`:120-128`、`:221-226`。
...
**脚本的三处修复历史**（`docs/测试报告.md:261-266`）—— 旧版跑不通，三处都在脚本侧：
...
修完实跑：**PASS 30 / SKIP 2 / FAIL 0**（同上 `:266`）。

> ⚠️ **该脚本跑完会留下测试残留** —— 本轮是**重跑整条重建链**才把库恢复成干净口径的，
> 所以**不要写"verify 跑完库仍是 1503 / 10719 / 19319.3"**
> （`docs/测试报告.md:267-268`）。

### 4.9.3 真后端逐页点检

2026-09-25 补做的浏览器级验证（`docs/测试报告.md:221-226`）：
...
项目把踩过的坑固化为两份清单，共 **30 条**（后端 20 条 + 前端 10 条），
写在根 `CLAUDE.md`（`:103-223` 后端、`:224-284` 前端）。这些是**实现阶段最有价值的沉淀**，
...
1. **工程实现**：11 个 Maven 模块、16 张表、12 个 SQL 脚本、29 个前端页面、70 个接口调用；
...
3. **业务模块实现**：6 个业务模块全部落地，接口规模 18/6/5/10（volunteer/certification/portrait/analytics）；
   实现阶段修掉了 **2 个必修缺陷**：
...
最后，本章如实收录了实现阶段积累的 **30 条踩坑清单**（后端 20 + 前端 10）。
这些坑的共同特征是"**不报错但结果是错的**" ——
