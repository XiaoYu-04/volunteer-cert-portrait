...
**接口规模（实测）**：后端 **59 条路径 / 72 个「方法 + 路径」**
（运行实例 `GET /v3/api-docs` 实查，2026-09-24）；前端 `src/api/` **70 个调用**；
差额 2 个是后端有、前端没调的 `POST /portraits/recompute` 与 `PUT /orgs/{id}`
（`docs/测试报告.md:16`、`docs/前端进展与待办.md:122-123`）。

> **前端调用数可复核**：`src/api/` 下 8 个文件共 70 个导出接口函数
> （`analytics.js` 10 / `attachment.js` 1 / `auth.js` 7 / `certification.js` 6 /
> `org.js` 4 / `portrait.js` 4 / `system.js` 20 / `volunteer.js` 18）。
...
4. **接口**：`/api/v1/` 前缀、`{code,message,data}` 外壳、9 类状态字典；
   实测后端 59 条路径 / 72 个操作，前端 70 个调用，差额 2 个已说明原因。
