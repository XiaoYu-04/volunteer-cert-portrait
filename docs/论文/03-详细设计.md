...
（`docs/测试报告.md:186-188`、`docs/后端进展与待办.md:436`）。
...
- **已知缺口**：`target`（操作对象）**写入侧从不填** —— `@OperationLog` 注解缺 `target` 属性
  （`docs/测试报告.md:202`、`docs/待办清单.md:869-870`）。
...
活动封面另存于 `volunteer_activity.cover`（`docs/测试报告.md:146-148`）。
...
（`docs/测试报告.md:297-299`、`docs/后端进展与待办.md:449-450`）。
...
| 规则 | 说明 | 出处 |
|---|---|---|
| 提交前置条件 | 报名状态必须是 `APPROVED` 或 `COMPLETED` | `docs/测试报告.md:174-177` |
...
**不伪装 PASS、也不算失败**"（`.tmp-verify/verify.ps1:106-117`、`docs/测试报告.md:265-266`）。
...
`AND status IN ('APPROVED','COMPLETED')`（`docs/测试报告.md:171-175`）。
...
（`docs/测试报告.md:295`、`CLAUDE.md` 工作方式约定）。
...
（`docs/测试报告.md:171-175`、`docs/后端进展与待办.md:433`）。

**实测证据**（`docs/测试报告.md:176-178`）：
...
**四个业务模块的接口规模**（`docs/后端进展与待办.md:237-242`）：

| 模块 | 路径 | 接口数 | 权限 |
|---|---|---|---|
| `vcp-volunteer` | `/activities`、`/categories`、`/signups`、`/attendance` | **18** | 活动/报名/签到各自授权；**签到签退仅需登录** |
...
**合计实测**：后端 **59 条路径 / 72 个「方法 + 路径」**；前端 `src/api/` **70 个调用**；
