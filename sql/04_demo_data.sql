...
--   报名 149 条、其中已完成 77 条
--   另生成对应签到记录 77 条、服务时长记录 69 条、审核流水 128 条、通知 210 条
--   （以上均为在 PostgreSQL 18.6 上实跑得到的实测值，非估算）
--
...
--
-- 报名带"偏好分类"：每个学生有 2 个偏好分类（由 id 推导），偏好该活动分类的
--   学生优先入选，使同一学生在偏好分类内反复参与。这既符合真实志愿者的行为，
--   也让"活动类型偏好"与类型标签有实际意义
--   （改为偏好之前是均匀打散，80 组里 79 组只参与过 1 次同分类活动）。
--
-- 公益等级与公益标签：规则已由团队确认，依据见 docs/公益等级与标签规则方案.md。
--   等级阈值采用"方案 A 演示尺度"，为便于答辩演示而刻意偏低，
--   实际部署时应按学校标准调整（改 04 末尾的 UPDATE 里的阈值即可）。
--
...
CROSS JOIN LATERAL (
    ...
    ORDER BY
        -- 每个学生有 2 个"偏好分类"（由 id 推导），偏好该活动分类的学生优先入选。
        -- 这样同一学生会在偏好分类内反复参与，符合真实志愿者行为，
        -- 也让"活动类型偏好"与类型标签有实际意义。
        -- （此前用均匀打散，导致 79/80 组只参与过 1 次同分类活动）
        CASE WHEN (si2.id % 6) + 1 = a.category_id
               OR ((si2.id + 2) % 6) + 1 = a.category_id THEN 0 ELSE 1 END,
        (si2.id * 13 + a.id * 29) % 37
    LIMIT 5 + (a.id % 8)
) s
...
-- 公益等级：按累计有效时长分档。阈值已由团队确认（方案 A 演示尺度），
-- 依据与取舍见 docs/公益等级与标签规则方案.md。
-- 本项目为便于答辩演示采用较低阈值，实际部署时应按学校标准调整。
UPDATE student_info
...
-- 九、学生公益画像
--
-- 规则已确认，依据与取舍见 docs/公益等级与标签规则方案.md。
--   （这两项规则在开发计划 / 知识库 / 选题对话三份文档中均未定义，属新增约定）
--   标签规则：
--     · 累计有效时长 > 0        → 「热心志愿者」
--     · 已完成活动次数 >= 3     → 「长期坚持型」
--       （阈值取 3 而非 5：演示数据里单个学生最多只有 4 次已完成活动，取 5 会导致该标签永不出现）
--     · 按参与次数最多的活动分类追加一个类型标签（互斥，不设参与次数下限）：
--         校园服务→校园服务型   社区服务→社区服务型   环保公益→环保行动型
--         大型赛事→大型活动型   助老服务→助老服务型   文化传播→文化传播型
--       （后两个标签名文档未给出，属新增 —— 否则这两类参与者拿不到类型标签，
--         而文化传播恰是人次最多的分类）
--     · 并列时按分类 id 升序打破，不用中文名排序：中文串的排序结果依赖数据库
