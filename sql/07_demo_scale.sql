...
SELECT n.activity_id,
       ...
       /* 名额：40..75。**必须大于第五节的报名数上限（39）**：
          signed_count 口径是「APPROVED + COMPLETED」，已结束活动约 85% 的报名
          会变成 COMPLETED，若名额比报名数小就会出现「已报名人数 > 名额」，
          末尾自检会直接失败（这也是报名数上限定在 39 而不是 47 的原因）。 */
       40 + ((n.seq * 7) % 36),
       -- 时长 2..6 小时
...
-- 五、追加报名
--
-- 每个活动取 8..39 人（约 24 人/场）。选人顺序用 md5(学生id || 活动id)，
...
-- 11.3 活动已报名人数：口径 = 审核通过 + 已完成（与 04 及「是否报满」判断一致）
