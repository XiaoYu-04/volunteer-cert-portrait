...
| 部分 | 状态 |
|---|---|
| 前端 `volunteer-cert-portrait-web` | ✅ 29 个页面全部完成（第 29 页「学院管理」为 2026-09-24 新增）；接口层 71 个调用与后端全部对得上，**联调已完成（B12，2026-09-23 审计通过）**。dev server 代理可配置（`VITE_API_TARGET`，默认 `http://127.0.0.1:8080`） |
...
**接口规模**：OpenAPI 共 60 个路径 / 73 个「方法 + 路径」（2026-09-26 运行实例 `/v3/api-docs` 实测），
前端 `src/api/` 的 71 个调用全部能对上，0 缺失。
（较 2026-09-24 多 1 条：B24 学生自助签到新增的 `GET /api/v1/attendance/mine`）
