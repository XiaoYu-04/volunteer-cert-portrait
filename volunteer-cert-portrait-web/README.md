...
打开 [http://localhost:5173](http://localhost:5173)。用下面三个账号登录（登录页不再提供「点击填入」的演示账号区，直接输入即可）：

| 角色 | 用户名 | 密码 | 登录后落地页 |
|---|---|---|---|
| 学生 | `student`（或任一学号，如 `202301010001`） | `123456` | `/student` |
| 组织管理员 | `org_admin` | `123456` | `/org` |
| 学校管理员 | `admin` | `123456` | `/admin` |

> 数据库演示数据（2026-09-27 起）：10 学院 / 1000 学生 / 10 名学校管理员 / 10 名组织管理员 /
> 11 个组织 / 10 场活动；学生的**用户名就是学号**。详见仓库根目录 `sql/README.md`。
